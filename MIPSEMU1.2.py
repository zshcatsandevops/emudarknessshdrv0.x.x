...
# -*- coding: utf-8 -*-
"""
MIPSEMU 1.1a — Enhanced MIPS R4300i + N64 Hardware Emulation
...
- Enhanced boot compatibility with HLE IPL3
- PI/SI DMA implementation for cartridge and PIF transfers
- ROM Doctor utility for byte-order normalization
- COP0 COUNT register ticking
"""
...
# ===========================================================================
# ROM Utilities (Enhanced with Z64 Doctor)
# ===========================================================================
def detect_rom_layout(data: bytes) -> str:
    """Detect ROM byte order: z64 (big), n64 (little), or v64 (byte-swapped)"""
    if len(data) < 4:
        return 'unknown'
...
def to_z64_bytes(data: bytes) -> bytes:
    """Convert any ROM format to z64 (big-endian) format"""
    layout = detect_rom_layout(data)
    if layout == 'z64':
        return data
    elif layout == 'n64':
        # Little-endian pairs to big-endian
...
def doctor_rom_file(in_path: str, out_path: str = None) -> str:
    """Normalize a ROM file to z64 format"""
    with open(in_path, 'rb') as f:
        raw = f.read()
    fixed = to_z64_bytes(raw)
    out_path = out_path or (str(Path(in_path).with_suffix('')) + '.z64')
...
# ===========================================================================
# Memory (Enhanced with DMA support)
# ===========================================================================
class Memory:
    __slots__ = (
        ...
        "pi_dram_addr", "pi_cart_addr", "si_dram_addr",
        "tlb"
    )
    ...
    def copy_from_rom(self, cart_addr: int, dram_addr: int, length: int):
        """Copy data from ROM cartridge to RDRAM (for DMA)"""
        if not self.rom or length <= 0:
            return 0
        src = cart_addr & 0x0FFFFFFF
        dst = dram_addr & 0x007FFFFF  # 8 MB RDRAM window
        n = max(0, min(length, self.rom_size - src, len(self.rdram) - dst))
        if n > 0:
            self.rdram[dst:dst+n] = self.rom[src:src+n]
        return n
    
    def _pi_dma_cart_to_dram(self, length_field: int):
        """PI DMA from cartridge to RDRAM"""
        # PI_RD_LEN write triggers DMA of (len + 1) bytes
        length = (length_field & 0x00FFFFFF) + 1
        copied = self.copy_from_rom(self.pi_cart_addr, self.pi_dram_addr, length)
        # Clear PI busy instantly (HLE); set MI/PI intr bits if you want later
        # PI status register is index 4 (0x04600010)
        self.pi_regs[4] &= ~0x03  # clear DMA busy/error (very simplified)
        return copied
    
    def _si_dma(self, to_pif: bool):
        """SI DMA between RDRAM and PIF RAM"""
        # N64 behavior (simplified):
        # - Write to SI_PIF_ADDR_RD64B (0x04800004) => PIF->RDRAM (64 bytes)
        # - Write to SI_PIF_ADDR_WR64B (0x04800010) => RDRAM->PIF (64 bytes)
        dram = self.si_dram_addr & 0x007FFFFF
        dram_end = min(dram + 64, len(self.rdram))
        if to_pif:
            # RDRAM -> PIF RAM
            self.pif_ram[:min(64, dram_end - dram)] = self.rdram[dram:dram_end]
        else:
            # PIF RAM -> RDRAM
            n = min(64, len(self.pif_ram))
            self.rdram[dram:dram+n] = self.pif_ram[:n]
        # Clear SI busy bits here if you later add them.
    ...
    def write_word(self, addr: int, value: int):
        ...
        # ---------------- PI registers (0x0460_0000) ----------------
        if 0x04600000 <= a < 0x04600040:
            idx = (a - 0x04600000) >> 2
            self.pi_regs[idx] = v
            if idx == 0:          # PI_DRAM_ADDR
                self.pi_dram_addr = v
            elif idx == 1:        # PI_CART_ADDR
                self.pi_cart_addr = v
            elif idx == 2:        # PI_RD_LEN -> start DMA Cart -> DRAM
                self._pi_dma_cart_to_dram(v)
            # idx == 3 (PI_WR_LEN) ignored for now (rarely used)
            # idx == 4 (PI_STATUS) would clear intr/busy bits; fine to store
            return
        
        # ---------------- MI registers (0x0430_0000) ----------------
        if 0x04300000 <= a < 0x04300010:
            self.mi_regs[(a - 0x04300000) >> 2] = v
            return
        
        # ---------------- SI registers (0x0480_0000) ----------------
        if 0x04800000 <= a < 0x04800020:
            ...
            if idx == 0:
                ...
                # SI_PIF_ADDR_RD64B (trigger PIF -> RDRAM)
                self._si_dma(to_pif=False)
            elif idx == 4:
                # SI_PIF_ADDR_WR64B (trigger RDRAM -> PIF)
                self._si_dma(to_pif=True)
            return
        
        # Hardware register writes (other regions)
...
# ===========================================================================
# PIF (Enhanced boot with HLE IPL3)
# ===========================================================================
class PIF:
    ...
    def simulate_boot(self, rom_header) -> bool:
        """HLE IPL3 boot sequence"""
        if not rom_header or not rom_header.valid:
            return False
        
        # 1) Mirror header to DMEM (as before)
        chunk = rom_header.raw_data[:0x1000]
        self.memory.sp_dmem[:len(chunk)] = chunk
        
        # 2) Minimal HLE IPL3:
        #    - Most ROMs place first-stage code at ROM offset 0x1000,
        #      linked to run at 0x80000400 (the header boot_address).
        #    - Copy ~1MB to get the loader running; game code will do
        #      proper PI DMAs afterwards.
        entry = rom_header.boot_address or 0x80000400
        dst_phys = entry & 0x1FFFFFFF
        copied = self.memory.copy_from_rom(0x00001000, dst_phys, 0x00100000)  # 1 MiB first stage
        
        # 3) Initialize PIF RAM (controllers connected)
        self.memory.pif_ram[0x24:0x28] = b'\x00\x00\x00\x3F'
        for i in range(4):
            self.controller_state[i] = 0x0500
...
# ===========================================================================
# MIPS R4300i CPU (Complete instruction set with COUNT ticking)
# ===========================================================================
class MIPSCPU:
    __slots__ = (
    ...
    def step(self):
        ...
        try:
            ...
            # Keep COUNT ticking (Patch C)
            cnt = (self.cop0.registers[COP0.REG_COUNT] + 1) & 0xFFFFFFFF
            self.cop0.registers[COP0.REG_COUNT] = cnt
            # Optional: raise timer interrupt when COUNT == COMPARE
            if cnt == self.cop0.registers[COP0.REG_COMPARE]:
                # Set IP7 timer bit in CAUSE (bit 15 = 0x8000)
                # self.cop0.registers[COP0.REG_CAUSE] |= 0x8000
                pass  # Not fully handled yet
...
# ===========================================================================
# Emulator UI (Enhanced with ROM Doctor)
# ===========================================================================
class N64Emulator:
    def __init__(self, root, engine_mode: str = ENGINE):
        self.root = root
        self.root.title("MIPSEMU 1.1a — Enhanced R4300i + N64")
    ...
    def _build_ui(self):
        ...
        # Tools menu with ROM Doctor
        tools_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Tools", menu=tools_menu)
        tools_menu.add_command(label="ROM Doctor (Normalize to .z64)", command=self._rom_doctor)
        ...
        self._log("MIPSEMU 1.1a — Enhanced MIPS R4300i + N64 Hardware")
        self._log("New in 1.1a:")
        self._log("  ✓ HLE IPL3 boot for better compatibility")
        self._log("  ✓ PI/SI DMA implementation (cart↔RDRAM, PIF↔RDRAM)")
        self._log("  ✓ COP0 COUNT register ticking")
        self._log("  ✓ ROM Doctor utility for byte-order normalization")
    ...
    def _rom_doctor(self):
        """ROM Doctor tool to normalize ROM byte order"""
        path = filedialog.askopenfilename(
            title="Select ROM to normalize",
            filetypes=[("N64 ROMs", "*.z64 *.n64 *.v64"), ("All Files", "*.*")]
        )
        if not path:
            return
        try:
            out = doctor_rom_file(path)
            self._log(f"ROM Doctor: wrote normalized file: {out}")
            messagebox.showinfo("ROM Doctor", f"Normalized ROM written:\n{out}")
        except Exception as e:
            self._log(f"ROM Doctor error: {e}")
            messagebox.showerror("ROM Doctor", f"Failed to normalize ROM:\n{e}")
    ...
    def load_rom(self, filepath: str):
        try:
            ...
            with open(filepath, "rb") as f:
                sample = f.read(4)
            layout = detect_rom_layout(sample)
            ...
            self._log(f"Format: {layout} ({hdr.endian}-endian)")
            ...
            if layout != 'z64':
                self._log(f"TIP: Use Tools → ROM Doctor to convert to .z64 format")
            
            self.root.title(f"MIPSEMU 1.1a — {hdr.name}")
    ...
    def start_emulation(self):
        try:
            ...
            self._log("=" * 64)
            self._log("STARTING BOOT SEQUENCE (HLE IPL3)")
            self._log("=" * 64)
...
def main():
    print("=" * 74)
    print("MIPSEMU 1.1a — Enhanced MIPS R4300i + N64 Hardware Emulation")
    print("=" * 74)
    print("\nFeatures:")
    ...
    print("\nNew in 1.1a:")
    print("  ✓ HLE IPL3 boot for better compatibility")
    print("  ✓ PI/SI DMA implementation (cart↔RDRAM, PIF↔RDRAM)")
    print("  ✓ COP0 COUNT register ticking")
    print("  ✓ ROM Doctor utility (Tools menu) for .n64/.v64 → .z64 conversion")
    print("=" * 74)
    print()
