...
Merged version with triangle rasterizer and complete feature set
"""
...
class MIPSEMU_PRO:
    def __init__(self, root):
        ...
        self.instructions_per_frame = 25000
    ...
    def render_framebuffer(self, x: int, y: int):
        ...
        for py in range(0, 240, step):
            for px in range(0, 320, step):
                color = self.rdp.framebuffer[py, px]
                hex_color = f"#{color[0]:02x}{color[1]:02x}{color[2]:02x}"
                self.canvas.create_rectangle(
                    x + px * scale, y + py * scale,
                    x + (px + step) * scale, y + (py + step) * scale,
